...
import { WorkersAiProvider, GroqProvider, GeminiProvider, AionProvider, type AiProvider } from "./lib/ai-providers";
...
export function createContainer(env: Env) {
  ...
  // AI providers — used only for age-rating classification. Pinned to
  // 70B-and-above models throughout — see lib/ai-providers.ts's model
  // constants. Aion is only invoked as a targeted fallback when an earlier
  // provider refuses on policy/safety grounds, never for quota/network
  // failures.
  ...
  return {
    ...
    ageRatingService: new AgeRatingService(stories, chapters, aiProviders, aion),
  };
}
